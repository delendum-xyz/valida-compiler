//===-- ValidaInstructionSelector.cpp -------------------------*- C++ -*-===//
//===----------------------------------------------------------------------===//
/// \file
...
#include "ValidaRegisterBankInfo.h"
...
#include "ValidaGenGlobalISel.inc"
#undef GET_GLOBALISEL_PREDICATE_BITSET

namespace {

class ValidaInstructionSelector : public InstructionSelector {
public:
  ValidaInstructionSelector(const ValidaTargetMachine &TM, 
                              const ValidaSubtarget &STI,
                              const ValidaRegisterBankInfo &RBI);
...
private:
  ...
  const ValidaInstrInfo &TII;
  const ValidaRegisterInfo &TRI;
  const ValidaRegisterBankInfo &RBI;
...
ValidaInstructionSelector::ValidaInstructionSelector(
    const ValidaTargetMachine &TM, const ValidaSubtarget &STI,
    const ValidaRegisterBankInfo &RBI)
...
int ValidaInstructionSelector::getFPOffset(MachineOperand &MO) const {
...
bool ValidaInstructionSelector::select(MachineInstr &I) {
...
bool ValidaInstructionSelector::customSelect(MachineInstr &I) const {
...
bool ValidaInstructionSelector::selectCall(MachineInstr &I) const {
  MachineBasicBlock &MBB = *I.getParent();
...
bool ValidaInstructionSelector::selectJump(MachineInstr &I) const {
...
bool ValidaInstructionSelector::selectArithmetic(MachineInstr &I,
                                                  int DstOffset) const {
  MachineInstr *MI = nullptr;
...
bool ValidaInstructionSelector::selectStore(MachineInstr &I) const {
  MachineInstr *MI = nullptr;
  using namespace TargetOpcode;
...
namespace llvm {
InstructionSelector *
createValidaInstructionSelector(const ValidaTargetMachine &TM,
                                  const ValidaSubtarget &Subtarget,
                                  const ValidaRegisterBankInfo &RBI) {
  return new ValidaInstructionSelector(TM, Subtarget, RBI);
}
} // end namespace llvm
